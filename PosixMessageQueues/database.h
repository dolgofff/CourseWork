...
private:
	...
	static string IPC_one_unit_parser(string& str) {

		string unit_value;
		size_t division1, division2;
		division1 = str.find('{');
		if (division1 != 0) {
			throw logic_error("PARSING ERROR no.4: Invalid unit format(begin sign).1");
		}

		division2 = str.find("} ");
		if (division2 == string::npos) {
			throw logic_error("PARSING ERROR no.4: IOUP Invalid unit format(end sign).");
		}

		unit_value = str.substr(1, division2 - 1);

		/*if (unit_value.empty()) {
			string error_text = "PARSING ERROR IPC: Invalid queue message unit";
			throw logic_error(error_text);
		}*/
	...
	static string IPC_one_unit_parser_last(string& str) {

		string unit_value;
		size_t division1, division2;
		division1 = str.find('{');
		if (division1 != 0) {
			throw logic_error("PARSING ERROR no.4: Invalid unit format(begin sign).2");
		}

		division2 = str.find('}');
		if (division2 == string::npos) {
			throw logic_error("PARSING ERROR no.4: IOUPL Invalid unit format(end sign).");
		}

		unit_value = str.substr(1, division2 - 1);
		/*if (unit_value.empty()) {
			string error_text = "PARSING ERROR no.5: Invalid last queue message unit";
			throw logic_error(error_text);
		}*/
	...
	public:
	//Принимает строку вида: [option] [key] [value]
	void option_parser(const string& option_key_value) {

		string option_key_value_t = option_key_value;

		string option = IPC_one_unit_parser(option_key_value_t);
		string key = IPC_one_unit_parser(option_key_value_t);
		string value = IPC_one_unit_parser_last(option_key_value_t);
...
//throw logic_error("PARSING ERROR no.8: Invlaid option string format(inside).");
            ...
            else if (nameof_allocator_str == "sorted list") {
                nameof_allocator = allocators::sorted_list_allocator;
            }
            else if (nameof_allocator_str == "buddies") {
                nameof_allocator = allocators::buddies_allocator;
            }
            else {
                throw logic_error("PARSING ERROR no.9: Invalid name of allocator.");
            }
            ...
            if (nameof_allocator == allocators::border_descriptors_allocator || nameof_allocator == allocators::sorted_list_allocator || nameof_allocator == allocators::buddies_allocator) {

                if (nameof_allocator == allocators::border_descriptors_allocator || nameof_allocator == allocators::sorted_list_allocator) {

                    division2 = str_cut.find(',');
                    if (division2 == string::npos) {
                        throw logic_error("PARSING ERROR no.8: Invalid option string format(inside).1");
                    }
                }

                else if (nameof_allocator == allocators::buddies_allocator){
                    division2 = str_cut.find(']');
                    if (division2 == string::npos) {
                        throw logic_error("PARSING ERROR no.8: Invalid option string format(inside).1");
                    }
                ...
                if (nameof_allocator != allocators::buddies_allocator) {
            ...
            else if(nameof_allocator == allocators::global_heap_allocator) {
                size = 1;
                division1 = str_cut.find(']');
                if (division1 != 0) {
                    throw logic_error("PARSING ERROR no.8: Invlaid option string format(inside).2");
                }
            }
            ...
            if (nameof_allocator == allocators::border_descriptors_allocator || nameof_allocator == allocators::sorted_list_allocator) {
                ...
                division2 = str_cut.find(']');
                if (division2 == string::npos) {
                    throw logic_error("PARSING ERROR no.8: Invalid option string format(inside).3");
                }
            ...
            else if(nameof_allocator == allocators::buddies_allocator || nameof_allocator == allocators::global_heap_allocator) {
                current_status = status::neutral;
                division1 = str_cut.find(']');
                if (division1 != 0) {
                    throw logic_error("PARSING ERROR no.8: Invalid option string format(inside).4");
                }
            }
			...
			else if (nameof_tree_str == "splay tree") {
				nameof_tree = trees::splay_tree;
			}
			else {
				throw logic_error("PARSING ERROR no.14: Invalid option string format(creating collection).");
			}
...
private:
	...
	void add_pool(const string& nameof_pool, allocators nameof_allocator, size_t size, status cur_status = status::neutral) {
		...
		if (nameof_allocator == allocators::border_descriptors_allocator) {
		...
		else if (nameof_allocator == allocators::sorted_list_allocator) {

			if (cur_status == status::best_available) {
				used_allocator = new allocator_sorted_list(size, allocator_sorted_list::status::best_available, _logger, nullptr);
			}
			else if (cur_status == status::worst_available) {
				used_allocator = new allocator_sorted_list(size, allocator_sorted_list::status::worst_available, _logger, nullptr);
			}
			else if (cur_status == status::first_available) {
				used_allocator = new allocator_sorted_list(size, allocator_sorted_list::status::first_available, _logger, nullptr);
			}
			else {
				throw logic_error("TECHNICAL ERROR no.6: Invalid allocation mode!");
			}
			...
			used_allocator = new allocator_buddies(size, _logger, nullptr);
		}
	...
	void add_collection(const string& nameof_collection, const string& nameof_pool, const string& nameof_scheme, trees tree_v) {
		...
		else if (tree_v == trees::splay_tree) {

			auto* cur_tree = reinterpret_cast<splay_tree<delivery_data, consumer_info, delivery_data_comparer>*>(used_allocator->allocate(sizeof(splay_tree<delivery_data, consumer_info, delivery_data_comparer>)));
			new (cur_tree) splay_tree<delivery_data, consumer_info, delivery_data_comparer>(used_allocator, _logger);
